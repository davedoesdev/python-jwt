...
def generate_jwt(claims, priv_key=None,
    ...
    :param priv_key: The private key to be used to sign the token. Note: if you pass :obj:`None` then the token will be returned with an invalid cryptographic signature (it will have the value ```*```). Use :obj:`None` when you're not using a public key infrastructure to authenticate your data.
    ...
    :param algorithm: The algorithm to use for generating the signature. ``RS256``, ``RS384``, ``RS512``, ``PS256``, ``PS384``, ``PS512``, ``ES256``, ``ES384``, ``ES512``, ``HS256``, ``HS384`` and ``HS512`` are supported.
    :type algorithm: str
    ...
    return "%s.%s.%s" % (
        ...
        jws.sign(header, claims, priv_key) if priv_key else '*'
    )
...
def verify_jwt(jwt, pub_key=None, iat_skew=timedelta()):
    ...
    header = jws.utils.from_json(header)
